create table if not exists public.session_rule_evaluations (
  ...
  skip_reason text,
  created_consequence_count integer not null default 0 check (created_consequence_count >= 0),
  created_task_count integer not null default 0 check (created_task_count >= 0),
  created_inject_count integer not null default 0 check (created_inject_count >= 0),
  context jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default timezone('utc', now()),
  created_by uuid references public.profiles (user_id) on delete set null
);

create index if not exists idx_session_rule_evaluations_session_id
on public.session_rule_evaluations (session_id, created_at desc);

create index if not exists idx_session_rule_evaluations_rule_id
on public.session_rule_evaluations (rule_template_id, created_at desc);

alter table public.session_rule_evaluations enable row level security;

drop policy if exists session_rule_evaluations_all on public.session_rule_evaluations;
create policy session_rule_evaluations_all on public.session_rule_evaluations
for all
to authenticated
using (public.can_access_session(session_id, auth.uid()))
with check (public.can_access_session(session_id, auth.uid()));

create or replace function public.log_session_rule_evaluation(
  p_session_id uuid,
  ...
  p_created_consequence_count integer default 0,
  p_created_task_count integer default 0,
  p_created_inject_count integer default 0,
...
as $$
begin
  insert into public.session_rule_evaluations (
    session_id,
    rule_template_id,
    session_inject_id,
    decision_id,
    ...
    matched,
    skip_reason,
    created_consequence_count,
    created_task_count,
    created_inject_count,
    context,
    created_by
  )
  values (
    ...
    p_task_id,
    p_event_type,
    coalesce(p_matched, false),
    nullif(trim(coalesce(p_skip_reason, '')), ''),
    greatest(coalesce(p_created_consequence_count, 0), 0),
    greatest(coalesce(p_created_task_count, 0), 0),
    greatest(coalesce(p_created_inject_count, 0), 0),
...
grant execute on function public.log_session_rule_evaluation(uuid, uuid, text, boolean, text, uuid, uuid, uuid, uuid, integer, integer, integer, jsonb) to authenticated;
