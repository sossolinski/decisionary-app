create index if not exists idx_session_tasks_due_runtime
on public.session_tasks (session_id, status, due_at);

create or replace function public.interpolate_runtime_template(p_template text, p_context jsonb default '{}'::jsonb)
returns text
language plpgsql
immutable
as $$
...
begin
  ...
  loop
    ...
    if p_event_type = 'task_overdue' then
...
grant execute on function public.interpolate_runtime_template(text, jsonb) to authenticated;
grant execute on function public.evaluate_session_rules(uuid, text, uuid, uuid, uuid, text, uuid) to authenticated;
grant execute on function public.process_overdue_session_tasks(uuid) to authenticated;
